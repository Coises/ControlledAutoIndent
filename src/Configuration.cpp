...
namespace {
    ...
    const char configFor[] = "ControlledAutoIndent";  // identifying string for this plugin
...
void loadConfiguration() {
    ...
    filePath += L"\\ControlledAutoIndent.json";
    ...
    if (saved.is_discarded()) {
        MessageBox(plugin.nppData._nppHandle,
            ...
            L"ControlledAutoIndent", MB_ICONWARNING);
        configIgnored = true;
        return;
    }
    ...
    if (configFound) {
        if (configIgnored) {
            if (MessageBox(plugin.nppData._nppHandle,
                ...
                L"ControlledAutoIndent", MB_YESNO | MB_ICONWARNING) == IDNO) return;
        }
        else {
            std::error_code ec;
            if (lastWriteTime != std::filesystem::last_write_time(filePath, ec)) {
                if (MessageBox(plugin.nppData._nppHandle,
                    ...
                    L"ControlledAutoIndent", MB_YESNO | MB_DEFBUTTON2 | MB_ICONWARNING) == IDNO) return;
            }
        }
    }
